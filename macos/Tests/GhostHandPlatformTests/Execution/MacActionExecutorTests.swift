...
@Suite struct MacActionExecutorTests {
    ...
    @Test func reactivatesTargetWhenGhostHandHoldsFocus() async throws {
        let h = Harness(frontmost: 1)  // GhostHand's own panel
        let result = try await h.run(.pressReturn)
        #expect(result.success)
    ...
    @Test func typeTextSetsAXValueWithoutKeystrokes() async throws {
        ...
        let result = try await h.run(.typeText, element, text: "GhostHand Automated Typing")
        #expect(result.success)
        #expect(h.ax.node(handle)?.value == "GhostHand Automated Typing")
        #expect(h.ax.setValues.map(\.value) == ["GhostHand Automated Typing"])
