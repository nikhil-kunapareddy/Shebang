...
@Suite struct VisionOCRServiceTests {
    ...
    @Test func recognizesRenderedTextAndMapsToScreen() throws {
        ...
        let results = try VisionOCRService().recognizeText(in: try render("GhostHand Test", size: window.size, scale: 1),
                                                           windowFrame: window)
        let text = try #require(results.first { $0.label.contains("GhostHand") })
