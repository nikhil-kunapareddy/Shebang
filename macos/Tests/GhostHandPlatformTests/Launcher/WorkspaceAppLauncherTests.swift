...
@Suite struct WorkspaceAppLauncherTests {
    ...
    @Test func AL03_extractURLLaunch_extractsHttpAndHttpsURLs() {
        let url = launcher.extractURLLaunch(from: "open https://github.com/dushyantzz/Ghosthand to check release")
        #expect(url?.absoluteString == "https://github.com/dushyantzz/Ghosthand")
    }
...
/// anything; run with `GHOSTHAND_LIVE_TESTS=1`.
@Suite(.enabled(if: ProcessInfo.processInfo.environment["GHOSTHAND_LIVE_TESTS"] == "1"))
