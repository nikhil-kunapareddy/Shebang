...
/// Read-only checks against the real frontmost app. Opt in with `SHEBANG_LIVE_UI_TESTS=1`; needs a GUI
/// session with Accessibility (and Screen Recording for the OCR check). Nothing is clicked or typed.
@Suite(.enabled(if: LiveUI.enabled, "Set SHEBANG_LIVE_UI_TESTS=1 to run live UI tests"))
