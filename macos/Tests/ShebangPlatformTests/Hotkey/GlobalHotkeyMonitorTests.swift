...
@Suite struct GlobalHotkeyMonitorTests {
    ...
    /// Installs a real event tap when Accessibility is granted; run with `SHEBANG_LIVE_TESTS=1`.
    @Test(.enabled(if: ProcessInfo.processInfo.environment["SHEBANG_LIVE_TESTS"] == "1"))
