        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("GhostHand").font(.title2.bold())
                    Text(model.summary).foregroundStyle(model.isReady ? Color.secondary : Color.orange)
                }
            }

            Text("Switch to any app and press ⌃⌘ (Control+Command) to tell GhostHand what to do. "
                 + "Press ⌃⌘ or Esc during a run to stop it. You can close this window; GhostHand stays in the menu bar.")
