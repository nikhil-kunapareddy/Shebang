...
/// `shebang check`: configuration, API key presence, permissions, then a live Jev call.
...
enum CheckCommand {
    static func execute(envFile: URL?) async -> Int32 {
        ...
        Console.line("  System: \(ShebangCLI.platformDescription)")
        if let envFile {
        ...
        if accessibility {
            ...
            Console.line("    then quit and reopen it. Shebang.app needs its own grant.")
        }
        ...
        do {
            ...
            if accessibility {
                ...
                Console.line("'shebang read' and 'shebang run'.")
            }
            return 0
