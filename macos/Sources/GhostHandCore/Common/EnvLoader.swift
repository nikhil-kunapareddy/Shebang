...
public enum EnvLoader {
    /// `$PWD/.env`, `~/Library/Application Support/GhostHand/.env`, then `.env` in the directory containing
    ...
    public static var defaultSearchPaths: [URL] {
        ...
        if let support = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            paths.append(support.appendingPathComponent("GhostHand", isDirectory: true).appendingPathComponent(".env"))
        }
