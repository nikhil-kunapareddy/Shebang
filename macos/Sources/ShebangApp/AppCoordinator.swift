...
final class AppCoordinator {
    ...
    private lazy var statusWindow: HostedWindowController = {
        let controller = HostedWindowController(title: "Shebang Status & Permissions", rootView: StatusView(model: statusModel))
    ...
    private lazy var apiKeyWindow: HostedWindowController = {
        let controller = HostedWindowController(title: "Shebang — AI Gateway API Key", rootView: APIKeyView(model: apiKeyModel))
    ...
    func start() {
        ...
        Log.app.info("Shebang started. Press Control+Command to activate.")
    }
    ...
    private func showPrompt() {
        ...
        // Capture before showing anything so Shebang never becomes the target.
    ...
    private func submit(goal: String) {
        ...
        guard let target = captured ?? fallbackTarget() else {
            prompt.model.showError("No app to control. Click the app you want Shebang to use, then press ⌃⌘.")
            return
        }
    ...
    /// The prompt never activates Shebang, but a window it opened might have; give focus back to the target.
