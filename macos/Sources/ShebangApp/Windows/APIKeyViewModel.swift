...
final class APIKeyViewModel: ObservableObject {
    ...
    var headline: String {
        reason == .manual ? "AI Gateway API Key" : "Connect Shebang to Vercel AI Gateway"
    }

    var explanation: String {
        switch reason {
        case .missingForRun:
            return "Shebang needs an API key before it can run. It uses the Jev model through Vercel AI Gateway."
        case .firstRun, .manual:
            return "Shebang uses the Jev model through Vercel AI Gateway to decide each step."
        }
    }
