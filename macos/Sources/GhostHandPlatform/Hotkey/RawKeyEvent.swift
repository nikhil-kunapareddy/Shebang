...
/// A platform-independent raw keyboard event received from the event tap.
...
public struct RawKeyEvent: Sendable, Equatable {
    ...
    /// Synthesized by GhostHand itself (e.g. the executor pressing Escape); ignored by the chord machine.
    public var isInjected: Bool
    public var timestampMs: UInt64

    public init(keyCode: UInt16, isKeyUp: Bool, isInjected: Bool = false, timestampMs: UInt64 = 0) {
    ...
    public var isChordModifier: Bool { isControl || isCommand }

    public static func keyDown(_ keyCode: UInt16, isInjected: Bool = false, timestampMs: UInt64 = 0) -> RawKeyEvent {
        RawKeyEvent(keyCode: keyCode, isKeyUp: false, isInjected: isInjected, timestampMs: timestampMs)
    }

    public static func keyUp(_ keyCode: UInt16, isInjected: Bool = false, timestampMs: UInt64 = 0) -> RawKeyEvent {
        RawKeyEvent(keyCode: keyCode, isKeyUp: true, isInjected: isInjected, timestampMs: timestampMs)
    }
}
...
extension RawKeyEvent {
    ...
    static func fromFlagsChanged(
        ...
        return RawKeyEvent(keyCode: keyCode, isKeyUp: !isDown, isInjected: isInjected, timestampMs: timestampMs)
    }
}
