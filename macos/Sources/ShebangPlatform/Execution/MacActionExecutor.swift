...
public final class MacActionExecutor: ActionExecutor, @unchecked Sendable {
    ...
    /// Verifies permission and that the target is still frontmost. When Shebang itself (or nothing) holds
    ...
    static let accessibilityRequiredMessage = "Accessibility permission is required to control other apps. "
        + "Enable Shebang in System Settings > Privacy & Security > Accessibility."
    ...
    static func inputFailure(_ what: String) -> ActionResult {
        .failed("Could not post \(what) events. Check that Shebang has Accessibility permission.")
    }
