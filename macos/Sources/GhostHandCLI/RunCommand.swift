...
/// `ghosthand run "<goal>"`: the full agent loop, wired like the app, with progress on stdout.
enum RunCommand {
    ...
    static func execute(_ arguments: RunArguments) async -> Int32 {
        ...
        Console.line(isLive ? "GhostHand Agent Loop: LIVE EXECUTION MODE" : "GhostHand Agent Loop: DRY-RUN MODE", .bold)
        ...
        if TargetSelection.looksLikeTerminal(target) {
            // Live keystrokes into the terminal running ghosthand would land in this process (or the shell).
            let refuse = isLive && (TargetSelection.isHostTerminal(target) || arguments.target == .frontmost)
            Console.line("[NOTE] Target '\(target.processName)' looks like the terminal running ghosthand.",
                         refuse ? .red : .yellow)
            if refuse {
        ...
        if !Permissions.isAccessibilityTrusted {
            ...
            Console.line("be read and live input will not reach other apps. See 'ghosthand check'.\n", .yellow)
        }
...
final class InterruptHandler {
    ...
    init(onInterrupt: @escaping () -> Void) {
        ...
        source = DispatchSource.makeSignalSource(signal: SIGINT, queue: DispatchQueue(label: "com.ghosthand.cli.sigint"))
